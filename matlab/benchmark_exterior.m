...
% =========================================================================
% 1. CARREGAMENTO DAS 90 TILES
% =========================================================================
basePath = '/Volumes/HDD 500GB/dados_sum_proj';
datasets = cell(90, 3); 
for i = 1:90
    imgName  = sprintf('dapi_sum_tile_%d.tif', i);
    maskName = sprintf('mask_max_tile_%d.tif', i);
    
    datasets{i, 1} = sprintf('Tile %d', i);
    datasets{i, 2} = fullfile(basePath, imgName);
    datasets{i, 3} = fullfile(basePath, maskName);
end

% =========================================================================
% 2. CARREGAR O FICHEIRO TSV 
% =========================================================================
fprintf('\n[1/2] A ler o ficheiro TSV Mestre com as anotações...\n');
...
fprintf('      ✅ TSV carregado!\n');

% =========================================================================
% 3. CONFIGURAÇÃO DE FEATURES E PARÂMETROS
% =========================================================================
feat_profiles = {
    '1D Int. DAPI', ...
    '1D Log(IntDAPI)', ...
...
% --- ACUMULADORES PARA A MATRIZ DE CONFUSÃO (90 Lâminas) ---
global_gt_all = cell(nF, 1); 
global_pr_all = cell(nF, 1); 
% -----------------------------------------------------------

warning('off', 'stats:gmdistribution:FailedToConvergeReps');
warning('off', 'stats:gmdistribution:FailedToConverge');
fprintf('\n===========================================================\n');
fprintf(' BENCHMARK DE FEATURES — cell_cycle_v12 (90 Imagens)\n');
fprintf('=============================================================\n');

% =========================================================================
% 4. CICLO PRINCIPAL DE AVALIAÇÃO
% =========================================================================
for d = 1:nD
    ...
    gt_img = zeros(size(instance_mask), 'uint8'); 
    
    tile_num = sscanf(ds_name, 'Tile %d');
 
    if isnumeric(gt_table.tile_num)
        linhas_da_tile = gt_table.tile_num == tile_num;
    else
        linhas_da_tile = strcmp(strtrim(string(gt_table.tile_num)), num2str(tile_num));
    end
    
    dados_tile = gt_table(linhas_da_tile, :);
    celulas_encontradas = height(dados_tile);
    pixels_pintados = 0;
    
    for idx = 1:celulas_encontradas
        cell_id = dados_tile.obj_num(idx);
        fase_texto = string(dados_tile.GT_label(idx)); 
        
        if contains(fase_texto, "G1", 'IgnoreCase', true)
            valor_fase = 1;
        elseif contains(fase_texto, "G2", 'IgnoreCase', true) || contains(fase_texto, "M", 'IgnoreCase', true)
            valor_fase = 2;
        else
            valor_fase = 0; 
        end
        
        mascara_desta_celula = (instance_mask == cell_id);
        gt_img(mascara_desta_celula) = valor_fase;
        
        if valor_fase > 0
            pixels_pintados = pixels_pintados + sum(mascara_desta_celula(:));
        end
    end
    
    fprintf('      [DEBUG] Encontradas %d células no TSV. Pixels mapeados no GT: %d\n', celulas_encontradas, pixels_pintados);
    % -------------------------------------------------------
    ...
    for nc = nc_vals
        ...
        for f = 1:nF
            ...
            try
                ...
                ev_km  = evaluate_mask_gt(r, r.km,  gt_img);
                ev_gm  = evaluate_mask_gt(r, r.gm,  gt_img);
                ev_ens = evaluate_mask_gt(r, r.ens, gt_img);
                ...
                if ~isnan(ev_km.accuracy)
                    ...
                    % EMPILHAR RESULTADOS PARA A MATRIZ
...
fprintf('\n>>> Melhor feature global (melhor entre KM/GM/ENS): %s (%.2f%%) <<<\n', all_names{idx}, best_glob_acc);
fprintf('===========================================================\n\n');

% =========================================================================
% 5. DESENHAR A MATRIZ DE CONFUSÃO REAL
% =========================================================================
...
if ~isempty(idx_vencedor)
    ...
    figure('Name', 'Matriz de Confusão Global (90 Imagens)', 'Color', 'w', 'Position', [400 300 550 450]);
    cm = confusionchart(gt_cat, pr_cat, ...
        'RowSummary','row-normalized', ...
        'ColumnSummary','column-normalized');
        
    cm.DiagonalColor = '#7C7AAC';
    cm.OffDiagonalColor = '#d8d6ed';
    cm.FontColor = 'black';
    
    cm.Title = sprintf('Matriz de Confusão Global (90 Lâminas) - %s\n%d Células Validadas | Exatidão: %.2f%%', melhor_ens_nome, sum(validos), melhor_ens_acc);
    
    % --- O MOMENTO DA VERDADE NA CONSOLA ---
    fprintf('\n==========================================================\n');
    fprintf(' VALORES REAIS DA MATRIZ DE CONFUSÃO (%s)\n', melhor_ens_nome);
    fprintf('==========================================================\n');
    disp(cm.Values);
    fprintf('TOTAL DE OBJETOS AVALIADOS NESTA MATRIZ: %d\n', sum(cm.Values(:)));
    fprintf('==========================================================\n\n');
end
